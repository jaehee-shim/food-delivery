...
1. 고객이 결제한다 (status : 예매 완료)
1. 예매가 완료되면 예매 내역이 my page에 전달된다
1. 영화관에서 나의 예매 정보로 티켓을 수령한다 (status 변경: 수령함)
1. 고객이 예약을 취소할 수 있다 (티켓 수령 전)
1. 예매가 취소되면 예매 정보가 사라진다
1. 고객이 예매 상태를 중간중간 조회한다

비기능적 요구사항
1. 트랜잭션
    1. 결제가 되지 않은 예매 건은 아예 거래가 성립되지 않아야 한다  Sync 호출 
1. 장애격리
